...
namespace MRR_CLG
{
    ...
    public class RRGame // : INotifyPropertyChanged
    {
        ...
        public RRGame(Database lDBConn)
        {
            ...
            ListOfCommands = new CommandList(this);
        ...
        public void ProcessMove(MoveCard p_movecard)  //MoveCard.tCardType p_card, int p_player )
        {
            ...
            switch (p_movecard.Type)
            {
                ...
                case MoveCard.tCardType.Forward3:
                    ...
                    int currentPhase = p_movecard.PhasePlayed;

                    switch (p_movecard.Type)
                    {
                        case MoveCard.tCardType.Back1:
                            if (OptionCards.GetOption(tOptionCardCommandType.ReverseGears, thisplayer, currentPhase) != null)
                            {
                            ...
                            break;
                        case MoveCard.tCardType.Forward1:
                            if (OptionCards.GetOption(tOptionCardCommandType.Brakes, thisplayer, currentPhase) != null)
                            {
                            ...
                            break;
                        case MoveCard.tCardType.Forward3:
                            if (OptionCards.GetOption(tOptionCardCommandType.FourthGear, thisplayer, currentPhase) != null)
                            {
        ...
        {
            Player blockingPlayer = WorkingPlayers.FirstOrDefault(wp => wp.CurrentPos.X == currentX && wp.CurrentPos.Y == currentY);
        ...
        public void SetupGame()
        {
            ...
            //LoadRobots();
            string boardname = LoadBoard();

            //string destFile = Path.GetDirectoryName(boardname) + "/CurrentBoard.jpg";
            //if (File.Exists(destFile))
            //{
            //    File.Delete(destFile);
            //}
            //File.Copy(boardname.Replace(".srx", ".jpg"), destFile);
            ...
            //int StartListCount = StartList.Count();
            ...
            foreach (Player thisplayer in AllPlayers)
            {
                ...
                if (thisSquare != null)
                {
                    ...
                    //CommandItem newCommand = new CommandItem(0, 0, thisplayer, 0, Direction.None, SquareAction.BTConnect, this);
                    //AddOneCommandToDB(newCommand, thisplayer.ID, thisplayer.ID);
                    robotCount++;
                }
                else
                {
            ...
            // mark all "connect" commands as "ready"
            //DBConn.Command("Update CommandList set StatusID=2 where Phase=0 and CommandTypeID=70 ;");
            //DBConn.Command("call procCommandUpdateStatus(-1,0);");
...
/*
        public string MakeCardJson(int playerID)
        {
            ...
            string result = jsonFromQuery(strSQL);
//            result = result.Replace("\"Cardlist["+playerID+"]\"", result1);
        ...
        {
            string strSQL = "select *, concat('Cardlist[',RobotID,']') Cards from viewRobots;";
            string result = jsonFromQuery(strSQL);
...
*/
        ...
        #region TestRun
        public bool LoadTestRun()
        {
            ListOfCommands.Clear();
            //ListOfCommands.SetPhase(1);
            ...
            DBConn.Command("Update CurrentGameData set GameState=7, PhaseCount=2, AutoExecute=0, RobotsActive=1, Message='Load PTO '; ");
            return false;
        }
        ...
        public string ExecuteTurn()
        {
            ...
            WorkingPlayers = new Players(AllPlayers);
            ...
            if (RulesVersion==1)
            {
                ...
                {
                    thiscard.Priority = -WorkingPlayers.GetPlayer(thiscard.Owner).Priority;
                }
...
//            ListOfCommands.AddCommand("Phase 1");
            ...
            // begin moves
            for (int RunningPhase = CurrentPhase + 1; RunningPhase < PhaseCount + 1; RunningPhase++)
            {
            ...
            foreach (Player thisplayer in PlayerDirections) // WorkingPlayers)
            {
            ...
            // {
            //     int maxd = WorkingPlayers.Max(wp=>wp.DamagePoints);
        ...
        public void AddCommandsToDatabase()
        {
            ...
            string strSQL = "Delete from CommandList where Turn=" + cTurn + " and Phase>" + CurrentPhase.ToString() + ";";
...
//            ListOfCommands.SetPhase(p_PhaseNumber);
            ...
            //IEnumerable<Player> activePlayers = WorkingPlayers.Where(ap=>(ap.Active ));
            //IEnumerable<BoardElement> playersSquare = WorkingPlayers.Where(ap => (ap.Active)).Select(ap => g_BoardElements.GetSquare(ap.CurrentPos.X, ap.CurrentPos.Y));
            /*
             * IEnumerable<Player> randomizers = WorkingPlayers.Where(ap => (ap.Active &&
            ...
            if (AllowOptions)
            {
                ...
                {
                    Player currentPlayer = WorkingPlayers.GetPlayer(currentCard.Owner);
            ...
            while (true)
            {
                ...
                if (CurrentAction == DamageSequence)
                {
                    ...
                    // 6 repeat
                    //IEnumerable<Player> liveplayers = WorkingPlayers.Where(wp => wp.IsRunning);
                    Players liveplayers = new Players();
                    foreach (Player thisplayer in WorkingPlayers.Where(wp => wp.IsRunning))
                    {
                ...
                {
                    Player thisplayer = WorkingPlayers.GetPlayer(thisaction.RobotID);
                    switch (thisaction.SquareAction)
                    {
                        ...
                        case SquareAction.TouchLastManFlag:
                            foreach (Player oneplayer in WorkingPlayers.Where(op => op.LastFlag > 0))
                            {
                        ...
                        case SquareAction.PlayOptionCard:
                            ...
                            //Player usingPlayer = WorkingPlayers.GetPlayer(usingcard.Owner);
                    ...
                    if ((OL2 != null) && (OL2.Count() > 0))
                    {
                        ...
                        foreach (var thisplayer in OL2)
                        {
                            ...
                            if (BM2.Count(bm => bm.RobotID == thisplayer.PlayerID) > 0)
                            {
                                ...
                                Player thisWorkingPlayer = WorkingPlayers.GetPlayer(thisplayer.PlayerID);
            ...
            while(true)
            {
                ...
                CommandList MovedCommands = new CommandList(this);
        ...
        public bool AddDamage(Player p_thisrobot, int p_Damage, Player p_DamagingRobot = null)
        {
            ...
            if (p_thisrobot.IsDead)
            {
                ...
                do
                {
                    ...
                    //AddFlag(WorkingPlayers.GetPlayer(pushedPlayer), 2); // pushing caused this player to die
...
*/
        ...
        public void DamageAtSquare(RobotLocation DamageSquare, Player CausedDamage) // new RobotLocation(0, X, Y, Damage)
        {
            ...
            //var PlayerJoin = from DS in DamageSquareList join WP in WorkingPlayers on DS equals WP.CurrentPos select new { WP, DS.Index };
        ...
        public void ProgramRandom()
        {
            //NextFunction = FunctionList.CreatingProgram;
            ProgramRandomThread();
            //// start thread to call function
            //Thread CommandThread = new Thread(new ThreadStart(ProgramRandomThread));
            //CommandThread.SetApartmentState(ApartmentState.STA);
            //CommandThread.Name = "ProgramRandomThread"; // +Name;
            //CommandThread.Start();
            //Thread.Sleep(1);

        }

        public void ProgramRandomThread()
        {
            foreach (Player thisplayer in AllPlayers.Where(ap => ap.IsRunning))
            {
                //if ((Properties.Settings.Default.UseSmartRandom) || (thisplayer.ComputerPlayer))
                if ((false) || (thisplayer.ComputerPlayer))
                    {
                        GenerateProgramForPlayer(thisplayer);
                }
                else
                {
                    GiveThisPlayerARandomHand(thisplayer);
                }
            }
        ...
        public void GiveThisPlayerARandomHand(Player thisplayer)
        {
            Random myrandom = new Random();
            foreach (MoveCard thiscard in GameCards)
            {
                thiscard.CurrentOrder = myrandom.Next(1, 84);
            }

            while (thisplayer.CardsPlayedCount < 5)
            {
                MoveCard thiscard;
                if (GameCards.Count(gc => ((gc.Owner == thisplayer.ID) && (gc.PhasePlayed == -1))) < 1)
                {
                    // pick a random card that no one else owns, or has played
                    thiscard = GameCards.OrderBy(gc => gc.CurrentOrder).First(gc => ((gc.Owner == -1) && (gc.PhasePlayed == -1)));
                    thiscard.Owner = thisplayer.ID;
                }
                else
                {
                    thiscard = GameCards.OrderBy(gc => gc.CurrentOrder).First(gc => ((gc.Owner == thisplayer.ID) && (gc.PhasePlayed == -1)));
                }
        ...
        public void BoardLoadFromDB(int sourceID)
        {
            string strSQL;
            ...
            MySqlConnector.MySqlDataReader actionReader = DBConn.Exec(strSQL);
            ...
            g_BoardElements = new BoardElementCollection();
            strSQL = "Select X,Y,SquareType,Rotation from BoardItems where BoardID=" + sourceID + ";";
            //Console.WriteLine(strSQL);
            MySqlConnector.MySqlDataReader reader = DBConn.Exec(strSQL);
            while (reader.Read())
            {
                //int boardID = (int)reader[0];
                //string boardname = (string)reader[1];
                int boardX = (int)reader["X"];
                int boardY = (int)reader["Y"];
                if (boardX + 1 > g_BoardElements.BoardCols) g_BoardElements.BoardCols = boardX + 1;
                if (boardY + 1 > g_BoardElements.BoardRows) g_BoardElements.BoardRows = boardY + 1;
                ...
                g_BoardElements.SetSquare(boardX, boardY,(SquareType)reader["SquareType"],(Direction)reader["Rotation"],boardSquareActions);
            }

            reader.Close();
        }

        public void BoardSaveToDB(int destinationID)
        {
            DBConn.Command("Delete from BoardItems where BoardID=" + destinationID + ";");
            DBConn.Command("Delete from BoardItemActions where BoardID=" + destinationID + ";");
            //  loop through cells
            //  loop through actions
            foreach(BoardElement thisSquare in g_BoardElements.BoardElements)
            {
                ...
                foreach (BoardAction thisAction in thisSquare.ActionList)
                {
                    ...
                    strSQL = "Update Boards set " +
                        " X=" + g_BoardElements.BoardCols.ToString() +
                        ", Y=" + g_BoardElements.BoardRows.ToString() +
                        ", GameType=" + (int)GameType +
                        ", TotalFlags=" + TotalFlags.ToString() +
                        ", LaserDamage=" + LaserDamage.ToString() +
